...
const BookletEditor = () => {
  ...
  const handleCanvasDrop = useCallback((e: React.DragEvent) => {
    ...
    if (!jsonStr) return;
    try {
      const { event, catColor, format, size, fontScale } = JSON.parse(jsonStr);
      ...
      const tileElements = buildEventTile(event, x, y, format, catColor, booklet.brand.colors[0], size || "normal", fontScale ?? 1);
